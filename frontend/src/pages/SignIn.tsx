...
import { Atom, AlertCircle, GraduationCap, PenSquare, ShieldCheck, ChevronRight } from 'lucide-react';
...
export function SignIn() {
  ...
  useEffect(() => {
    if (!user) return;
    // Authors + admins land in the author console; students in the student app.
    const target =
  ...
  // Group by role for visual ordering
      ...
      <div className="max-w-3xl mx-auto">
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <p className="mt-2 text-slate-600 text-sm max-w-md">
            One-click dev login. The backend is seeded with users for each role so you can test every flow without typing.
          </p>
        </div>
        ...
        <p className="mt-8 text-center text-xs text-slate-500">
          Dev-only one-click login. Real auth (Google OAuth / password) lands in a later stage.
        </p>
      </div>
    </div>
  );
